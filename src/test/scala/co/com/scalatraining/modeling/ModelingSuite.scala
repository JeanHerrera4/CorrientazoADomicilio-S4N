...
class ModelingSuite extends FunSuite {
  ...
  test("testing smart constructor"){
    ...
    val OrientacionN = Try(newOrientacion('N'))
    val OrientacionS = Try(newOrientacion('S'))
    val OrientacionE = Try(newOrientacion('E'))
    val OrientacionO = Try(newOrientacion('O'))
    val OrientaccionInvalida = Try(newOrientacion('.'))

    assert(OrientacionN == Success(N()))
    assert(OrientacionS == Success(S()))
    assert(OrientacionE == Success(E()))
    assert(OrientacionO == Success(O()))
    assert(OrientaccionInvalida.isFailure)
